...
public class GLRotationHandler {
    ...
    public class Render_ID{
        public final static int ID_ORIGIN = 0;
        public final static int ID_TARGET = 1;
    }

    /**
     * 设置原始纹理和输出目标纹理大小，目标大小有变化，再重新设置一次
     * @param originWidth
     * @param originHeight
     * @param targetWidth
     * @param targetHeight
     */
    public void setTargetSize(int originWidth, int originHeight, int targetWidth, int targetHeight) {
        // 创建新的纹理ID
        release();
        if (mProgram == 0) {
            ...
            // 配置新纹理
            GLES20.glBindTexture(GLES20.GL_TEXTURE_2D, textures[Render_ID.ID_ORIGIN]);

            GLES20.glTexImage2D(GLES20.GL_TEXTURE_2D, 0, GLES20.GL_RGBA, originWidth, originHeight, 0, GLES20.GL_RGBA, GLES20.GL_UNSIGNED_BYTE, null);
            GLES20.glTexParameteri(GLES20.GL_TEXTURE_2D, GLES20.GL_TEXTURE_MIN_FILTER, GLES20.GL_LINEAR);
            GLES20.glTexParameteri(GLES20.GL_TEXTURE_2D, GLES20.GL_TEXTURE_MAG_FILTER, GLES20.GL_LINEAR);
            GLES20.glBindTexture(GLES20.GL_TEXTURE_2D, 0);

            GLES20.glBindTexture(GLES20.GL_TEXTURE_2D, textures[Render_ID.ID_TARGET]);

            GLES20.glTexImage2D(GLES20.GL_TEXTURE_2D, 0, GLES20.GL_RGBA, targetWidth, targetHeight, 0, GLES20.GL_RGBA, GLES20.GL_UNSIGNED_BYTE, null);
            GLES20.glTexParameteri(GLES20.GL_TEXTURE_2D, GLES20.GL_TEXTURE_MIN_FILTER, GLES20.GL_LINEAR);
            GLES20.glTexParameteri(GLES20.GL_TEXTURE_2D, GLES20.GL_TEXTURE_MAG_FILTER, GLES20.GL_LINEAR);
            GLES20.glBindTexture(GLES20.GL_TEXTURE_2D, 0);
    ...
    /**
     * 围绕X轴翻转，因为火山引擎不识别纹理坐标系，翻转为图片坐标系
     * @param originalTextureId
     * @return
     */
    public int rotateTextureAroundXAxis(int originalTextureId) {
        //第一次翻转，使用客户的宽高
        return rotateTextureAroundXAxis(originalTextureId, Render_ID.ID_TARGET, mTargetWidth, mTargetHeight);
    }
     ...
     * @return
     */
    public int rotateTextureBack(int originalTextureId) {
        //第二次翻转回来，使用原始的宽高
        return rotateTextureAroundXAxis(originalTextureId, Render_ID.ID_ORIGIN, mOriginWidth, mOriginHeight);
    }

    private int rotateTextureAroundXAxis(int originalTextureId, int targetId, int width, int height) {

        int[] old_fbo = new int[1];
        GLES20.glGetIntegerv(GLES20.GL_FRAMEBUFFER_BINDING, old_fbo, 0);

        int newTextureId = textures[targetId];
        int fbo = frameBuffers[targetId];
